...
public class GradesClass extends JDialog {
    ...
    private void onCompute() {
        ...
        Student_Info student = new Student_Info(lastName, firstName, middle, course, year);
        ...
        Compute_Grades grades = new Compute_Grades(lastName, firstName, middle, course, year);
        grades.setGrades(subject, prelim, midterm, finals);

        avg = grades.getAvg();
        averageTxt = decimal.format(avg);
        status = grades.checkGrade(avg);
        equivalent = grades.convert(avg);
